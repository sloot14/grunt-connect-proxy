...
utils.interceptCookieDomain = function(res, rewritePath) {
    function replaceDomain(value){
        return value.toString().replace(/domain=([^;]*)(;|$)/gi, function(match, path, end){
            return 'domain=' + rewritePath + end;
        });
    }
    ...
    res.setHeader = function(name, value) {
        if (name && value && name.toLowerCase() === 'set-cookie') {
            if (Array.isArray(value)) {
                value.forEach(function(input, i){
                    value[i] = replaceDomain(input);
                })
            } else {
                value = replaceDomain(value)
            }
            return _setHeader('Set-Cookie', value);
        } else {
...
function onUpgrade(req, socket, head) {
    ...
    proxies.forEach(function(proxy) {
        if (!proxied && req && proxy.config.ws && utils.matchContext(proxy.config.context, req.url)) {
            ...
            if(proxy.config.rewriteCookieDomain) {
                 utils.interceptCookieDomain(res, proxy.config.rewriteCookieDomain);
            }
